...
export default function SessionDetailPage() {
  ...
  useEffect(() => {
    async function load() {
      const [sRes, lRes] = await Promise.all([
        fetch(`/api/telehealth/sessions/${params.sessionId}`),
        fetch(`/api/telehealth/sessions/${params.sessionId}/logs`)
      ]);
  ...
  const updateStatus = async (newStatus: string) => {
    await fetch(`/api/telehealth/sessions/${params.sessionId}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ status: newStatus })
    });
    // Reload
    const res = await fetch(`/api/telehealth/sessions/${params.sessionId}`);
